...
# Shane McLeod, 2022-Dec-12, Added code for TODOs
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        CD_Track_Inv = file_name[1]
        try:
            with open(file_name_CD, 'w') as file:
                for disc in lst_Inventory:
                    file.write(disc.get_record())
            with open(CD_Track_Inv, 'w') as file:
                for disc in lst_Inventory:
                    for tracks in disc.cd_tracks:
                        file.write('{},{}'.format(disc.cd_id, tracks.get_record()))
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(CD_Track_Inv, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    cdInfo = PC.DataProcessor.select_cd(lst_Inventory, data[0])
                    trackInfo = DC.Track(data[1], data[2], data[3])
                    cdInfo.add_track(trackInfo)
